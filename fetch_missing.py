...
def check_and_download(fid):
    url = f'https://drive.google.com/uc?id={fid}'
    try:
        r = requests.head(url, allow_redirects=True)
        cd = r.headers.get('Content-Disposition')
        if cd:
            fn = get_filename_from_cd(cd).lower()
            if 'plain' in fn and any(t in fn for t in targets):
                print(f'Downloading {fn}...')
                gdown.download(url, output='temp_maps/'+fn, quiet=False)
    except:
        pass
