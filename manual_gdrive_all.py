...
targets = ['blizzard', 'lijiang', 'route']
...
def check_and_download(file_id):
    ...
    try:
        ...
        if fname:
            filename = fname[0].strip('\"\'').lower()
            # Since these 3 maps might not have _plain, we just take the first one that has _anno
            # Actually just any matching the target. If multiple, we might overwrite, which is fine for now
            if any(t in filename for t in targets) and ('anno' in filename or 'plain' in filename):
                print(f"Match found: {filename}")
                dest = os.path.join('temp_missing', filename)
        ...
        print("Error with", file_id, e)

with ThreadPoolExecutor(max_workers=5) as executor:
    executor.map(check_and_download, drive_ids)
